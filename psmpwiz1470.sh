...
VERSION_PSMP="v14.7" # UPDATE THIS (This is just for UI sake, it doesn't impact anything in the script)
scriptVersion="1" # Script version, only dev should update this.
...
newVersionFile="CARKpsmp-14.1.4.4.x86_64.rpm"                #update this locally
newIntergratedInfraFile="CARKpsmp-infra-14.1.4.4.x86_64.rpm" #update this locally

#packagenames (this goes with every -qa command)
newVersion="CARKpsmp-14.4.4-4.x86_64"       #UPDATE this to the latest version always (It's usually diff than the .rpm file we define above, it has dash instead of dot.)
newVersionSha256="e8d1ac94fae0f0fcdf95759f19b3b8e86606401b3501f0fc520bfbd3a6d9f894" # UPDATE this, example command: sha256sum CARKpsmp-14.1.1.4.x86_64.rpm | awk '{print $1}' 
...
#PVWA Calls
pvwaLogin() {
    rest=$(curl --location -k -m 40 --connect-timeout 15 -s --request POST --w " %{http_code}" "$pvwaURLAPI/Auth/CyberArk/Logon" \
...
pvwaLogoff() {
    pvwaActivate=$(
        curl --location -k -m 40 --connect-timeout 15 -s -d "" --request POST --w "%{http_code}" "$pvwaURLAPI/Auth/Logoff" \
            --header "Content-Type: application/json" \
...
pvwaGetUserId() {
    pvwaGetUser=$(
        curl --location -k -m 40 --connect-timeout 15 -s --request GET --w " %{http_code}" "$pvwaURLAPI/Users?filter=componentUser\&search=$credUsername&UserType=PSMPServer" \
            --header "Content-Type: application/json" \
            --header "Authorization: $pvwaHeaders"
    )
}
...
pvwaActivateUser() {
    pvwaActivate=$(
        curl --location -k -m 40 --connect-timeout 15 -s -d "" --request POST --w "%{http_code}" "$pvwaURLAPI/Users/$userID/Activate" \
            --header "Content-Type: application/json" \
            --header "Authorization: $pvwaHeaders"
    )
}
...
pvwaResetPW() {
    pvwaReset=$(curl --location -k -m 40 --connect-timeout 15 -s --request POST --w " %{http_code}" "$pvwaURLAPI/Users/$userID/ResetPassword" \
        --header "Content-Type: application/json" \
        --header "Authorization: $pvwaHeaders" \
        --data @<(
            cat <<EOF
...
pvwaSystemHealthUser() {
    pvwaSystemHealth=$(
        curl --location -k -m 40 --connect-timeout 15 -s --request GET --w " %{http_code}" "$pvwaURLAPI/ComponentsMonitoringDetails/SessionManagement" \
            --header "Content-Type: application/json" \
            --header "Authorization: $pvwaHeaders"
    )
}
        ...
        if rpm -qa | grep -q $newVersion; then
            ...
            if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
                ...
                if [[ $response =~ ^([cC|[cC])$ ]]; then
                    echo "This capability was moved to createCredFile-Helper.sh you can find it under 'Cyberark PrivilegeCloud Tools\CreateCredFile-Helper' folder"
					exit 1
                fi
            else
                exit
...
maintenanceUsers
    ...
    if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
        ...
        timeout 15 bash -c '</dev/tcp/'$vaultip'/1858' && echo port is open | cap || echo port is closed | cap
        ...
        if [[ "echo $ret" == *"open"* ]]; then
			...
			if [ ! -f "$VLTFILE" ]; then
			################################### Vault.ini
				if [ -f /etc/opt/CARKpsmp/vault/vault.ini ]; then
					echo "Deleting previous file /etc/opt/CARKpsmp/vault/vault.ini"
					rm -rf /etc/opt/CARKpsmp/vault/vault.ini
				fi
				echo "Applying vault address to vault.ini file..."
				sed -i "s/^\(ADDRESS=\).*/\1$vaultip/" vault.ini
				sed -i "s/^\(TIMEOUT=\).*/\1$vaultIniTimeout/" vault.ini
			...
			if [[ $pvwaURL == *".cyberark.cloud"* ]]; then
				...
				if [[ -n $location_url ]]; then
					...
					if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
						...
						# Test connection 
						if curl --output /dev/null --silent --head --fail --connect-timeout 15 "$location_url"; then
...
declare -a dirs=("/var/opt/CARKpsmp/logs/old")
